...
#if (defined OBCDT)
use openbc_mod, only: STARTLON, STARTLAT
#endif
!
...
!      REAL(r8)    :: ZKP (KMP1)
      ...
      REAL(r8)    :: ivk(imt_global,jmt_global)
      INTEGER :: IREC
!
!
#ifdef SPMD
...
#if (defined PPM)
      if (mytid==0)then
      write(6,*)"Use PPM advection scheme!"
      endif
#else 

#if (defined PRM)
      if (mytid==0)then
      write(6,*)"Use PRM advection scheme!"
      endif
#else
      if (mytid==0)then
...
!
      ...
      DO i = 1,imt_global
         lon(i) = STARTLON + (i-1)*DLAM

      END DO
#else
!
      DO i = 1,imt_global
!lhl20150914         lon (i)= 0.0+ (i -1)*DLAM
         lon (i)= 98.0+ (i -1)*DLAM
      END DO
...
#ifdef SPMD
!        if (mytid==0) then
!               write(121,*)"OK-----0", imt_global,jmt_global,km
!               close(121)
!        end if
...
!        end if
      ...
      if (mytid==0) then
!      iret=nf_open('yy.nc',nf_nowrite,ncid)
!
!        The Following Line Is Added by DFS in 2018-10
!
      iret=nf_open('yy_eas.nc',nf_nowrite,ncid)
!
!          DFS in 2018-10
!
      call check_err (iret)
      start1(1)=1 ; count1(1)=jmt_global
      iret=nf_inq_varid(ncid,   'yy',varid)
      call check_err (iret)
      iret=nf_get_vara_real(ncid,   varid,start1,count1,dyr_in_global)
      call check_err (iret)
      iret = nf_close (ncid)
      call check_err (iret)
!
      iret=nf_open('zz.nc',nf_nowrite,ncid)
      call check_err (iret)
      start1(1)=1 ; count1(1)=kmp1
      iret=nf_inq_varid(ncid,   'zz',varid)
      call check_err (iret)
      iret=nf_get_vara_real(ncid,   varid,start1,count1,zkp_in)
      call check_err (iret)
      iret = nf_close (ncid)
      call check_err (iret)
!      print*,dyr_in_global
...
!
      if (mytid==0) then
!      iret=nf_open('ind_final.nc',nf_nowrite,ncid)
!
!         The Following Line Is Added By DFS in 2018-10
!
      iret=nf_open('ind_final_eas.nc',nf_nowrite,ncid)
!
!         DFS in 2018-10
!
      call check_err (iret)
      ...
      do k=1,km
         if (mytid == 0) then
            ...
            iret=nf_inq_varid(ncid,   'ind',varid)
            call check_err (iret)
            iret=nf_get_vara_real(ncid,   varid,start3,count3,buffer_real4)
            call check_err (iret)

            do j=1,jmt_global
            do i=1,imt_global
               VIT_GLOBAL(i,j)=buffer_real4(i,j)
            end do
            end do
!
!
!#if (undefined OBCDT)
            ...
            do i=1,imt_global
              VIT_GLOBAL(i,1)= 0.0
              VIT_GLOBAL(i,2)= 0.0
              VIT_GLOBAL(i,jmt_global-1)= 0.0
              VIT_GLOBAL(i,jmt_global)= 0.0
            end do
            do j=1, jmt_global
               VIT_GLOBAL(1,j)= 0.0
               VIT_GLOBAL(2,j)= 0.0
               VIT_GLOBAL(imt_global-1,j)= 0.0
               VIT_GLOBAL(imt_global,j)= 0.0
            end do
#else
            do i=1,imt_global
...
!lhl20110803
            if (k.eq.1) then
               do j=1,jmt_global
               do i=1,imt_global
                  vit_global_surface(i,j)=vit_global(i,j)
               end do
               end do
            endif
!lhl20110803
            ...
            do j = 1,jmm_global
            do i = 2,imt_global
               viv_global(i,j)= vit_global(i-1,j)*vit_global(i-1,j+1)* &
                         vit_global(i,j)* vit_global(i,j+1)
            end do
               viv_global(1,j)= 0.0
!               viv_global(1,j)= viv_global (imt_global-1,j)
            end do

            do i = 1,imt_global
               viv_global(i,jmt_global)= 0.0
            end do
#else
            do j = 1,jmm_global
            do i = 2,imt_global
         ...
         call mpi_barrier(mpi_comm_ocn,ierr)
         call global_distribute(vit_global(1,1),vit(1,1,k))
         call global_distribute(viv_global(1,1),viv(1,1,k))
         call mpi_barrier(mpi_comm_ocn,ierr)
...
!        The Following Line Is Added by DFS in 2018-10
!
      iret=nf_open('BASIN_EAS.nc',nf_nowrite,ncid)
!
!        DFS in 2018-10
!      
      call check_err (iret)
!
      ...
      iret=nf_inq_varid(ncid,   'ind',varid)
      call check_err (iret)
      iret=nf_get_vara_int(ncid,   varid,start2,count2,basin)
      call check_err (iret)
      iret = nf_close (ncid)
      call check_err (iret)
!      print*,basin
...
!         The Following Line Is Added By DFS in 2018-10
!
      iret=nf_open('INDEX.DATA_EAS.nc',nf_nowrite,ncid)
!
!         DFS in 2018-10
!
      call check_err (iret)
...
!lhl20150715
      DO J = 1,JMT
         DO I = 1,IMT
            IUNU (I,J)= 0
            DO K = 1,KM
               IF (VIV (I,J,K) > 0.0) IUNU (I,J)= IUNU (I,J) +1
            END DO
         END DO
      END DO
!lhl20150715
...
!
      ...
      WKJ_global(1)= (90.0d0-STARTLAT)*TORAD

#else
!lhl20150914      WKJ_global(1)= 0.0D0
      WKJ_global(1)= 24.0D0*TORAD
!
#endif
!
!DFS
!
      ...
      END DO
!      if (mytid.eq.0)  print*, WKJ_global
!
!lhl060506
      DO J = 1,JMT_global
...
#else
!lhl20150914         AJQ = 90.0D0-(WKJ_global (J) + WKJ_global(J +1))*0.5D0/ TORAD
         AJQ = 66.0D0-(WKJ_global (J) + WKJ_global(J +1))*0.5D0/ TORAD
!
! DFS
!
#endif
...
!
#if (defined OBCDT)
      IF (WKJ_global(1).EQ.0.0D0) THEN
         SINT_global (1) = 1.0D-25
      ELSE
         SINT_global(1) = SIN(WKJ_global (1))
      ENDIF
      ...
      END DO
#else
!lhl20150914      DO J = 2,JMT_global
!lhl20150914         SINT_global(J) = SIN(WKJ_global (J))
!lhl20150914      END DO
!lhl20150914      SINT_global (1) = 1.0D-25
      DO J = 1,JMT_global
         SINT_global(J) = SIN(WKJ_global (J))
      END DO
#endif
...
!
#ifdef SPMD
      call exchange_2d(hby,1,1)
      call exchange_2d(hbx,1,1)
#endif
!
      DO J = 1,JMT_global
...
!            ASEA = ASEA + DYT_global(J)* SINT_global(J)* VIT_global(I,J,1)
            ASEA = ASEA + DYT_global(J)* SINT_global(J)* VIT_global_surface(I,J)
         END DO
      END DO
...
!           DFS in 2018-10
!
!lhl20150914      iret=nf_open('INDEX.DATA',nf_nowrite,ncid)
      call check_err (iret)
...
!#if (undefined OBCDT)
         ...
         DO J = 1,JMT_global
            DO I = 2,IMM_global
               VSEA = VSEA + DZP(K)* DXDYT_global(J)* VIT_global(I,J)
            END DO
         END DO
      END DO
...
!
      ...
      WKJ(1) = ( 90.0-STARTLAT ) * TORAD

#else
!lhl20150914      WKJ (1)= 0.0
      WKJ (1)= 24.0
...
#else
!lhl20150914         AJQ = 90.0D0- (WKJ (J) + WKJ (J +1))*0.5D0/ TORAD
         AJQ = 66.0D0- (WKJ (J) + WKJ (J +1))*0.5D0/ TORAD
...
!
!               ABCD = 40.0D0* TORAD
!               IF (ASIN (SINU (J)) >= ABCD) AH3(I,J,K)=2.0D+3
!
!#else
               AH3 (I,J,K)= 1.0D+3
!#endif
...
!
!#if (defined ACOS)
!           AH3 (I,J,K)= 2.0D+5*SINT(J)
!
!           ABCD = 40.0D0* TORAD
!           IF (ASIN (SINU (J)) >= ABCD) AH3(I,J,K)=2.0D+3*SINT(J)
!#else
!           AH3 (I,J,K)= 2.0D+5
!
!           ABCD = 40.0D0* TORAD
...
!      ABCD = 40.0D0* TORAD
!      DO J = 1,JMT
!         IF (ASIN (SINU (J)) >= ABCD) AM(J)= AM(J)*AM_TRO
!      END DO
!
!#else
