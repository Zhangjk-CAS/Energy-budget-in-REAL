...
!$OMP PARALLEL DO PRIVATE (J)
     do j = 2, jmm
         wka (1,j,13)= wka (2,j,13)
         wka (1,j,14)= wka (2,j,14)
         wka (imt,j,13)= wka (imm,j,13)
         wka (imt,j,14)= wka (imm,j,14)
     end do
!$OMP PARALLEL DO PRIVATE (I)
     do i = 1, imt
         wka (i,1,13)= wka (i,2,13)
         wka (i,1,14)= wka (i,2,14)
         wka (i,jmt,13)= wka (i,jem,13)
         wka (i,jmt,14)= wka (i,jem,14)
     end do
     end if
#else
     if (nx_proc == 1) then 
...
!
#ifdef  SPMD 
       call exchange_2d(wka(1,1,13),1,1)
       call exchange_2d(wka(1,1,14),1,1)
#endif
!
!$OMP PARALLEL DO PRIVATE (J,I)
      DO j = jst,jmt
...
!
         ...
         END DO
#ifdef  SPMD                                                   
       call exchange_2d(dst(1,1,1),1,1)                       
       call exchange_2d(dst(1,1,2),1,1)                       
#endif 
