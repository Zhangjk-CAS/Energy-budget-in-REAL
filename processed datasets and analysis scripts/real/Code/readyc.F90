...
!$OMP PARALLEL DO PRIVATE (K,J)
         do k = 1, km
         do j = jsm, jem
              wp12(1,j,k) = wp12(2,j,k)
              wp12(imt,j,k) = wp12(imm,j,k)
              wp13(1,j,k) = wp13(2,j,k)
              wp13(imt,j,k) = wp13(imm,j,k)
         end do
         end do
...
!$OMP PARALLEL DO PRIVATE (K,I)
         do k = 1, km
         do i = 1, imt
              wp12(i,1,k) = wp12(i,2,k)
              wp12(i,jmt,k) = wp12(i,jem,k)
              wp13(i,1,k) = wp13(i,2,k)
              wp13(i,jmt,k) = wp13(i,jem,k)
         end do
         end do
      end if
#else
...
!lhl0711#ifdef SPMD 
!lhl0711      call exchange_3d(wp12,km)
!lhl0711      call exchange_3d(wp13,km)
!lhl0711#endif      
...
!$OMP PARALLEL DO PRIVATE (K,J)
         ...
         do  j= jst,jet
            s2t(1,j,k) = s2t(2,j,k)
            s2t(imt,j,k) = s2t(imm,j,k)
            rit(1,j,k)=rit(2,j,k)
            rit(imt,j,k)=rit(imm,j,k)
            ridt(1,j,k)=ridt(2,j,k)
            ridt(imt,j,k)=ridt(imm,j,k)
         end do
         end do
...
!$OMP PARALLEL DO PRIVATE (K,J)
         ...
         do  i= 1, imt
            s2t(i,1,k) = s2t(i,2,k)
            s2t(i,jmt,k) = s2t(i,jem,k)
            rit(i,1,k) = rit(i,2,k)
            rit(i,jmt,k) = rit(i,jem,k)
            ridt(i,1,k) = ridt(i,2,k)
            ridt(i,jmt,k) = ridt(i,jem,k)
         end do
         end do
      endif
#else
...
!
!lhl0711#ifdef SPMD
!lhl0711      call exchange_3d(s2t,kmm1)
!lhl0711      call exchange_3d(rit,kmm1)
!lhl0711      call exchange_3d(ridt,kmm1)
!lhl0711#endif
#ifdef DEBUG
...
!
!lhl0711#ifdef SPMD
!lhl0711      call exchange_3d(riu,kmm1)
!lhl0711#endif
#ifdef DEBUG
...
!$OMP PARALLEL DO PRIVATE (J)
         do j = jsm, jem
              amld(1,j) = amld(2,j)
              amld(imt,j) = amld(imm,j)
         end do

!$OMP PARALLEL DO PRIVATE (I)
         do i = 1, imt
              amld(i,1) = amld(i,2)
              amld(i,jmt) = amld(i,jem)
         end do

!$OMP PARALLEL DO PRIVATE (K,J)
         DO K = 1, KM
         do j = jsm, jem
              akmt(1,j,k) = akmt(2,j,k)
              akmt(imt,j,k) = akmt(imm,j,k)
              akt(1,j,k,1) = akt(2,j,k,1)
              akt(imt,j,k,1) = akt(imm,j,k,1)
              akt(1,j,k,2) = akt(2,j,k,2)
              akt(imt,j,k,2) = akt(imm,j,k,2)
         END DO
         END DO
...
!$OMP PARALLEL DO PRIVATE (K,I)
         DO K = 1, KM
         do i = 1, imt
              akmt(i,1,k) = akmt(i,2,k)
              akmt(i,jmt,k) = akmt(i,jem,k)
              akt(i,1,k,1) = akt(i,2,k,1)
              akt(i,jmt,k,1) = akt(i,jem,k,1)
              akt(i,1,k,2) = akt(i,2,k,2)
              akt(i,jmt,k,2) = akt(i,jem,k,2)
         END DO
         END DO
     end if
#else
...
#ifdef SPMD
      call exchange_3d(amld,1,1,1)
      call exchange_3d(akmt,km,1,1)
...
!$OMP PARALLEL DO PRIVATE (K,J)
           do k = 1, kmm1
           do j = jsm, jem
              AKMU(1,j,k) = AKMU(2,j,k)
              AKMU(imt,j,k) = AKMU(imm,j,k)
           end do
           end do
...
!$OMP PARALLEL DO PRIVATE (K,I)
           do k = 1, kmm1
           do i = 1, imt
              AKMU(i,1,k) = AKMU(i,2,k)
              AKMU(i,jmt,k) = AKMU(i,jem,k)
           end do
           end do
      end if
#else
...
#ifdef SPMD
      call exchange_3d(akmu(1,1,1),km,1,1)
#endif
#ifdef DEBUG
...
!
       ...
       CALL ADVEC_PPM_UV( U, V, WKA, U, DLU )

       CALL ADVEC_PPM_UV( U, V, WKA, V, DLV )

#else
 
#if (defined PRM)

      CALL ADVEC_PRM_UV( U, V, WKA, U, DLU )

      CALL ADVEC_PRM_UV( U, V, WKA, V, DLV )
...
!dfs            if (k>=IUNU(I,J)) then
!               diff_v2= WKA (I,J,IUNU(I,J))* ( - SNLAT (J)* UP (I,J,IUNU(I,J))        &
!                        * SAG + VP (I,J,IUNU(I,J))* CAG)*(1-AIDIF)
!               diff_u2= WKA (I,J,IUNU(I,J))* ( UP (I,J,IUNU(I,J))* CAG + SNLAT (J)    &
!dfs                        * VP (I,J,IUNU(I,J))* SAG)*(1-AIDIF)
!lhl20150914
            if (k==km) then
               diff_v2= WKA (I,J,KM)* ( - SNLAT (J)* UP (I,J,KM)        &
                        * SAG + VP (I,J,KM)* CAG)*(1-AIDIF)
...
#ifdef SPMD
      call exchange_3d(akmu(1,1,1),km,1,0)
#endif
...
!
!lhl0711#ifdef SPMD
!lhl0711      call exchange_3d(akmu,km)
!lhl0711#endif
      deallocate(riu) 
...
!$OMP PARALLEL DO PRIVATE (K,J)
         do k = 1, km
         do j = jsm, jem
            TMP1 (1,J,K) = TMP1 (2,J,K)
            TMP1 (IMT,J,K)= TMP1 (IMM,J,K)
            TMP2 (1,J,K)= TMP2 (2,J,K)
            TMP2 (IMT,J,K)= TMP2 (IMM,J,K)
        end do
        end do
...
!$OMP PARALLEL DO PRIVATE (K,I)
         do k = 1, km
         do I = 1, IMT
            TMP1 (I,1,K) = TMP1 (I,2,K)
            TMP1 (I,JMT,K)= TMP1 (I,JEM,K)
            TMP2 (I,1,K)= TMP2 (I,2,K)
            TMP2 (I,JMT,K)= TMP2 (I,JEM,K)
        end do
        end do
      end if
#else
...
!YU
#ifdef SPMD
      call exchange_3d(tmp1,km,1,1)
      call exchange_3d(tmp2,km,1,1)
#endif 
!YU
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
...
!$OMP PARALLEL DO PRIVATE (K,J)
      DO K = 1,KM
         DO J = JSM,JEM
            DLV (1,J,K) = DLV (2,J,K)
            DLU (1,J,K) = DLU (2,J,K)
            DLV (IMT,J,K) = DLV (IMM,J,K)
            DLU (IMT,J,K) = DLU (IMM,J,K)
         END DO
      END DO
...
!$OMP PARALLEL DO PRIVATE (K,I)
      DO K = 1,KM
         DO I = 1, IMT
            DLV (I,1,K) = DLV (I,2,K)
            DLU (I,1,K) = DLU (I,2,K)
            DLV (I,JMT,K) = DLV (I,JEM,K)
            DLU (I,JMT,K) = DLU (I,JEM,K)
         END DO
      END DO
      end if
#else
