...
!  DFS 2024-05
!
      N2MONTH=RESHAPE((/31,29,31,30,31,30,31,31,30,31,30,31/),(/12/))
!      NN2MONTH=RESHAPE((/15,46,75,106,136,167,197,228,259,289,320,350/),(/12/))
!
!  DFS 2024
!
!-------------------------------------------------------
...
!-------------------------------------------------------
      if (mytid==0) then
      open(33,file='dncoef.h',form='formatted')
      read(33,*)
      read(33,*)to
      read(33,*)
      read(33,*)so
...
!YU
!lhl20150914      if ((imt_global-2)/=nint(360./DLAM)) then

!lhl20150914!lhl0711      if ((imt-2)>=nint(360./DLAM)) then
!lhl20150914         write(6,*)"Error in DLAM ! (Const)"
!lhl20150914         write(6,*)imt-2,nint(360./DLAM)
!lhl20150914         stop
!lhl20150914      end if
...
!zhangjk 250505
      if (mytid.eq.0) then
      iret=nf_open("../force-era/slpp-2013-2024.nc",nf_nowrite,ncid)
      call check_err (iret)
      iret=nf_get_var_double(ncid,3,s_lon)
      call check_err (iret)
      iret=nf_get_var_double(ncid,4,s_lat)
      call check_err (iret)
      iret=nf_close(ncid)
      call check_err (iret)
      do j=1,s_jmt
...
!zhangjk250528
!read lon lat for runoff 
      if (mytid.eq.0) then
      iret=nf_open("../force-era/rivr_climatological.nc",nf_nowrite,ncid)
      call check_err (iret)
      iret=nf_get_var_double(ncid,1,s_lon_ro)
      call check_err (iret)
      iret=nf_get_var_double(ncid,2,s_lat_ro)
      call check_err (iret)
      iret=nf_close(ncid)
      call check_err (iret)
      do j=1,s_jmt_ro
      tmpy(j)=s_lat_ro(s_jmt_ro-j+1)
      enddo
      do j=1,s_jmt_ro
      s_lat_ro(j)=tmpy(j)
      enddo
