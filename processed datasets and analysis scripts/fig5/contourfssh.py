...
pnstr = 'ssh-eg-glorys-win'
# key_str1 = '12.1.2-5year-z0.nc' # str1 for ej and jl ; str2 for jg 
key_str1 = '12.1.2-5year-z0.nc'
# key_str2 = '12.1.2-5year-aviso-10km.nc'
# key_str2 = '6.7.8-5year-aviso-10km.nc'
key_str2 = '12.1.2-5year-z0.nc'
vn = 'ssh'
# pathstr = ["../ave-TH/eg/", "./ssh/"]
...
for path in pathstr:
    ...
    for item in sorted(os.listdir(nowdir)):
        ...
        if key_str in item and '.png' not in item:
            ...
            #     var0 = -var0
            if number_var == 0:
                var = np.squeeze(var0[0,0,lat_loc[0]:lat_loc[1], lon_loc[0]:lon_loc[1]])
            else:
                var = np.squeeze(var0[0,lat_loc[0]:lat_loc[1], lon_loc[0]:lon_loc[1]]) - 0.2 
                # 0.2 for glorys;0.6 for AVISO
            var[var.mask] = np.nan
