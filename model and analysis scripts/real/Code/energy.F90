...
!!$OMP PARALLEL DO PRIVATE (J,I),reduction(+:EB)
      DO J = JSM,JMM
         DO I = 2,IMM
!            if(AT(I,J,1,1).GT.400 .OR. AT(I,J,1,1).LT.-400 ) THEN
!                iglerro=ix*(imt-num_overlap)+i
!                jglerro=jst_global-1+iy*(imt-num_overlap)+j
!                WRITE(*,*) I, J, iglerro, jglerro, AT(I,J,1,1)
!            ENDIF
...
!!$OMP PARALLEL DO PRIVATE (K,J,I),reduction(+:ET,ES)
      DO K = 1,KM
         DO J = JSM,JMM
            DO I = 2,IMM
                if(AT(I,J,K,1).GT.400 .OR. AT(I,J,K,1).LT.-400 .OR. &
                   isnan(AT(I,J,K,1)) ) THEN
                   iglerro=ix*(imt-num_overlap)+i
                   jglerro=jst_global-1+iy*(jmt-num_overlap)+j
                   WRITE(*,*) I, J,iglerro, jglerro, K, AT(I,J,K,1)
                ENDIF
!                if(AT(I,J,K,2).GT.400 .OR. AT(I,J,K,2).LT.-400 ) THEN
!                   WRITE(*,*) I, J, K, AT(I,J,K,2)
!                ENDIF
