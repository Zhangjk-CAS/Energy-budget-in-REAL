...
!
!   Dfs Added For Use In Open Boundary in Jan 2017
#if (defined OBCDT)
use openbc_mod
#endif
!   End
...
!#else
!
!               BBCX(I,J)= C0F*SQRT(UP(I,J,KM)*UP(I,J,KM)+VP(I,J,KM)*VP(I,J,KM)+0.0025)&
!                          *(UP(I,J,KM)*CAG+SNLAT(J)*VP (I,J,KM)*SAG)
!               BBCY(I,J)= C0F*SQRT(UP(I,J,KM)*UP(I,J,KM)+VP(I,J,KM)*VP(I,J,KM)+0.0025)&
!                          *(-SNLAT(J)*UP(I,J,KM)*SAG+VP(I,J,KM)*CAG)
!#endif
!
!
...
#ifdef SPMD
      call exchange_3d(dlu,km,1,0)
      call exchange_3d(dlv,km,1,0)
#endif
!YU  Oct. 24, 2005
...
#ifdef OBCDT
!--------------------------------------------------------------------
!OPEN BOUNDARY CONDITON WHEN ISC < 1
!ADD BY ZHANGJK 20250315
!--------------------------------------------------------------------
! #ifdef SPMD
!       call exchange_3d(u,km,1,1)
!       call exchange_3d(v,km,1,1)
! #endif
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         DO J = JST,JET
...
! add exchange 
! #ifdef SPMD
!    call exchange_3d_obc(up,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
!    call exchange_3d_obc(vp,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
!    call exchange_3d_obc(unow,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
!    call exchange_3d_obc(vnow,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
! #endif
...
! add exchange 
! #ifdef SPMD
!    call exchange_3d_obc(u,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
!    call exchange_3d_obc(v,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
! #endif
...
#ifdef OBCDT
!--------------------------------------------------------------------
!OPEN BOUNDARY CONDITON WHEN ISC > 1 --V
!ADD BY ZHANGJK 20250315
!--------------------------------------------------------------------
! #ifdef SPMD
!       call exchange_3d(wka,km,1,1)
! #endif
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         DO J = JST,JET
            DO I = 1,IMT
               VNOW (I,J,K)= VP (I,J,K) + DLV (I,J,K)* DTC2
            END DO
         END DO
      END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         DO J = JST,JET
            DO I = 1,IMT
               UNOW (I,J,K)= UP (I,J,K) + DLU (I,J,K)* DTC2
            END DO
         END DO
      END DO
...
! add exchange 
#ifdef SPMD
   call exchange_3d_obc(wka,km,4,4,imt-2,imt-2,jmt-3,jmt-3,3,3)
#endif
...
#ifdef OBCDT
!--------------------------------------------------------------------
!OPEN BOUNDARY CONDITON WHEN ISC > 1 --U
!ADD BY ZHANGJK 20250315
!--------------------------------------------------------------------
! #ifdef SPMD
!       call exchange_3d(wka,km,1,1)
...
!lhl0711     IF (MOD(ISC,60)==0) THEN
     IF (MOD(ISC,160)==1) THEN
        ...
        call exchange_3d(u,km,1,1)
        call exchange_3d(v,km,1,1)
        call exchange_3d(up,km,1,1)
        call exchange_3d(vp,km,1,1)
#endif
     END IF
!YU 
