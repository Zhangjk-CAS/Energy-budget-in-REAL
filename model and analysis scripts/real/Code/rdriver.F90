...
!
!     Define Variables.
!lhl20150914      integer*4   :: ncid1, iret
      integer*4   :: ncid1, iret,varid
...
!
      iret=nf_open('MODEL.FRC_EAS.nc',nf_nowrite,ncid1)
!
!         DFS in 2018-10
!
      call check_err (iret)
...
!----------------------------------------------------
      ...
      if (mytid ==0 ) then
         iret=nf_inq_varid(ncid1,   'sw',varid)
         call check_err (iret)
         iret=nf_get_vara_real(ncid1,   varid,start,count,buffer_real4)
         call check_err (iret)
      end if
      call global_distribute_real(buffer_real4,swv3(1,1,k))
!
      if (mytid == 0) then
         iret=nf_inq_varid(ncid1,   'nsw',varid)
         call check_err (iret)
         iret=nf_get_vara_real(ncid1,   varid,start,count,buffer_real4)
         call check_err (iret)
      end if
...
!
      if (mytid == 0) then
         iret=nf_inq_varid(ncid1,   'dqdt',varid)
         call check_err (iret)
          iret=nf_get_vara_real(ncid1,   varid,start,count,buffer_real4)
          call check_err (iret)
      end if
...
!
      if (mytid == 0 ) then
         iret=nf_inq_varid(ncid1,   'us',varid)
         call check_err (iret)
          iret=nf_get_vara_real(ncid1,   varid,start,count,buffer_real4)
          call check_err (iret)
      end if
...
!
      if (mytid == 0 ) then
         iret=nf_inq_varid(ncid1,   'vs',varid)
         call check_err (iret)
          iret=nf_get_vara_real(ncid1,   varid,start,count,buffer_real4)
          call check_err (iret)
      end if
...
!
      if (mytid == 0) then
         iret=nf_inq_varid(ncid1,   'sst',varid)
         call check_err (iret)
          iret=nf_get_vara_real(ncid1,  varid,start,count,buffer_real4)
          call check_err (iret)
      end if
...
!
      if (mytid == 0 ) then
         iret=nf_inq_varid(ncid1,   'sss',varid)
         call check_err (iret)
         iret=nf_get_vara_real(ncid1,  varid,start,count,buffer_real4)
         call check_err (iret)
      end if
...
!input seaice
      if (mytid == 0 ) then
!      iret=nf_open('seaice.db.NSIDC.1979-2006.clim.monthmean.modelgrid.01x01_np_20150914.nc',nf_nowrite,ncid1)
!
!          The Following Line Is Added by DFS in 2018-10 to Instead of the Above Line
!
      iret=nf_open('seaice.db.NSIDC.1979-2006.clim.monthmean.modelgrid.01x01_eas.nc',nf_nowrite,ncid1)
!
!           DFS in 2018-10
!
      call check_err (iret)
      ...
      start(4)=k ; count(4)=1
      if (mytid == 0 ) then
      iret=nf_inq_varid(ncid1,   'seaice',varid)
      call check_err (iret)
      iret=nf_get_vara_double(ncid1, varid,start,count,buffer)
      call check_err (iret)
      end if
...
!input runoff
      if (mytid == 0 ) then
!      iret=nf_open('runoff.db.clim.modelgrid.01x01_np_20150914.nc',nf_nowrite,ncid1)
!
!          The Following Line Is Added by DFS In 2018-10 to Instead of the Above Line
!
      iret=nf_open('runoff.db.clim.modelgrid.01x01_eas.nc',nf_nowrite,ncid1)
!
!          DFS in 2018-10
!
      call check_err (iret)
...
!
      ...
      start(4)=k ; count(4)=1
      if (mytid == 0 ) then
      iret=nf_inq_varid(ncid1,   'RUNOFF',varid)
      call check_err (iret)
      iret=nf_get_vara_double(ncid1, varid,start,count,buffer)
      call check_err (iret)
      end if
...
!        The Following Line Is Added by DFS in 2018-10 to Instead of The Above Line
!
      OPEN (90,FILE ='MODEL.FRC_EAS.nc',STATUS ='OLD',FORM ='UNFORMATTED')
!
!           DFS in 2018-10
!
