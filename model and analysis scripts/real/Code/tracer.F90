...
#endif
!
! Dfs Added For Use In Open Boundary Condition in 2016-12-9
#if (defined OBCDT)
use openbc_mod
#endif
! dfs
...
#if (defined TSPAS)
      real(r8)    :: LAMDAN,LAMDAS,wt1,wt2,adv_z
...
! DFS
#if (defined PPM)
      real(r8)    :: LAMDAN,LAMDAS,wt1,wt2

!      INTEGER :: KUT
      REAL(r8), DIMENSION(IMT,JMT,KM) :: UPPM, VPPM

      REAL(r8), ALLOCATABLE :: STTMP(:,:,:)
      REAL(r8), ALLOCATABLE :: AXTMP(:,:,:), AYTMP(:,:,:), AZTMP(:,:,:)
#else 
...
!      INTEGER :: KUT
      REAL(r8), DIMENSION(IMT,JMT,KM) :: UPRM, VPRM
...
! DFS
      ...
      real(r8)    :: LAMDAN,LAMDAS,wt1,wt2,adv_y,adv_x,adv_z,adv_x1,adv_x2
#endif
#endif
#endif
!XC
...
!      Define the threthold latitute for zonal smoother
      ...
      LAMDAN=1.0D0/(30.D0*86400.D0)
      LAMDAS=1.0D0/(10.D0*86400.D0)
...
!$OMP PARALLEL DO PRIVATE (J)
            DO J = JSM, JEM
               ...
               uaa (IMT,J,K) = uaa(IMM,J,K)
               vaa (IMT,J,K) = vaa(IMM,J,K)
            ENDDO
!$OMP PARALLEL DO PRIVATE (I)
            DO I = 1, IMT
               ...
               uaa (I,JMT,K) = uaa(I,JEM,K)
               vaa (I,JMT,K) = vaa(I,JEM,K)
            ENDDO
         END DO
         end if
#else
...
#ifdef SPMD
       call exchange_3d(uaa(1,1,1),km,1,1)
       call exchange_3d(vaa(1,1,1),km,1,1)
#endif
#endif
!XC
!
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1, KM
         DO J = 1, JMT
            DO I = 1, IMT
               UPPM(I,J,K) = WKD (I,J,K)
               VPPM(I,J,K) = WKC (I,J,K)
...
!         DO J = JSM,JEM
!            DO I = 2,IMM
!               UPPM(I,J,K)=0.5D0*WKD (I,J,K) + 0.5D0*WKD (I,J-1,K)
!               VPPM(I,J,K)=0.5D0*WKC (I,J,K) + 0.5D0*WKC (I+1,J,K)
!            END DO
!         END DO
!      END DO
!
...
!$OMP PARALLEL DO PRIVATE (K,J)
         DO K = 1, KM
         DO J = JST, JET
               UPPM (1,J,K) = UPPM(IMM,J,K)
               UPPM (IMT,J,K) = UPPM(2,J,K)
               VPPM (1,J,K) = VPPM(IMM,J,K)
               VPPM (IMT,J,K) = VPPM(2,J,K)
         END DO
         END DO
...
!$OMP PARALLEL DO PRIVATE (K,I)
         DO K = 1, KM
         DO I = 1, IMT
               UPPM (I,1,K) = UPPM(I,2,K)
               UPPM (I,JMT,K) = UPPM(I,JEM,K)
               VPPM (I,1,K) = VPPM(I,2,K)
               VPPM (I,JMT,K) = VPPM(I,JEM,K)
         END DO
         END DO
         end if
#else
...
!
#ifdef SPMD
       call exchange_3d(UPPM(1,1,1),KM,1,1)
       call exchange_3d(VPPM(1,1,1),KM,1,1)
#endif
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1, KM
         DO J = 1, JMT
            DO I = 1, IMT
               UPRM(I,J,K) = WKD (I,J,K)
               VPRM(I,J,K) = WKC (I,J,K)
            END DO
         END DO
      END DO
!
...
!         DO J = JSM,JEM
!            DO I = 2,IMM
!               UPRM(I,J,K)=0.5D0*WKD (I,J,K) + 0.5D0*WKD (I,J-1,K)
!               VPRM(I,J,K)=0.5D0*WKC (I,J,K) + 0.5D0*WKC (I+1,J,K)
!            END DO
!         END DO
!      END DO
!
...
!$OMP PARALLEL DO PRIVATE (K,J)
         DO K=1,KM
         DO J = JST,JET
               UPRM (1,J,K) = UPRM(2,J,K)
               UPRM (IMT,J,K) = UPRM(IMM,J,K)
               VPRM (1,J,K) = VPRM(2,J,K)
               VPRM (IMT,J,K) = VPRM(IMM,J,K)
         END DO
         END DO
...
!$OMP PARALLEL DO PRIVATE (K,I)
         DO K=1,KM
         DO I = 1, IMT
               UPRM (I,1,K) = UPRM(I,2,K)
               UPRM (I,JMT,K) = UPRM(I,JEM,K)
               VPRM (I,1,K) = VPRM(I,2,K)
               VPRM (I,JMT,K) = VPRM(I,JEM,K)
         END DO
         END DO
         end if
#else
...
!$OMP PARALLEL DO PRIVATE (K,J)
         DO K=1,KM
         DO J = JST,JET
               UPRM (1,J,K) = UPRM(IMM,J,K)
               UPRM (IMT,J,K) = UPRM(2,J,K)
               VPRM (1,J,K) = VPRM(IMM,J,K)
               VPRM (IMT,J,K) = VPRM(2,J,K)
         END DO
         END DO
         end if
#endif
...
!
#ifdef SPMD
       call exchange_3d(UPRM(1,1,1),KM,1,1)
       call exchange_3d(VPRM(1,1,1),KM,1,1)
#endif
...
#if (!defined CANUTO)
#ifdef SPMD
      call exchange_3d(akt(1,1,1,1),km,1,1)
      call exchange_3d(akt(1,1,1,2),km,1,1)
#endif
#endif
!
#ifdef DEBUG
...
!$OMP PARALLEL DO PRIVATE (K,J)
      DO K = 1, km
        DO J = JSM, JEM
           atmax(1  ,j,k) = atmax(2,j,k)
           atmin(IMT,j,k) = atmin(IMM  ,j,k)
        ENDDO
      ENDDO
...
!$OMP PARALLEL DO PRIVATE (K,I)
      DO K = 1, km
        DO I = 1, IMT
           atmax(I,1,k) = atmax(I,2,k)
           atmin(I,JMT,K) = atmin(I,JEM,k)
        ENDDO
      ENDDO
     end if
#else
...
#ifdef SPMD
       call exchange_3d(atmax,km,1,1)
       call exchange_3d(atmin,km,1,1)
#endif
...
#ifdef SPMD
       call exchange_3d(ax(1,1,1,n),km,1,1)
       call exchange_3d(ay(1,1,1,n),km,1,1)
#endif
...
#ifdef SPMD
       call exchange_3d(az(1,1,1,n),km,1,1)
#endif
...
!
       ...
       IF ( ALLOCATED( STTMP ) )  DEALLOCATE( STTMP )
       ALLOCATE( STTMP(IMT,JMT,KM) )

       IF ( ALLOCATED( AXTMP ) )  DEALLOCATE( AXTMP )
       ALLOCATE( AXTMP(IMT,JMT,KM) )

       IF ( ALLOCATED( AYTMP ) )  DEALLOCATE( AYTMP )
       ALLOCATE( AYTMP(IMT,JMT,KM) )

       IF ( ALLOCATED( AZTMP ) )  DEALLOCATE( AZTMP )
       ALLOCATE( AZTMP(IMT,JMT,KM) )

        DO I = 1, IMT
           DO J = 1, JMT
              DO K= 1, KM

                 STTMP(I,J,K) = AT(I,J,K,N)
       ...
       CALL ADVEC_PPM_TS( UPPM, VPPM, WS, STTMP, N, TF, AXTMP, AYTMP, AZTMP )
       
        DO I =1, IMT
           DO J =1, JMT
              DO K =1, KM
                 ax(I,J,K,N) = AXTMP(I,J,K)
                 ay(I,J,K,N) = AYTMP(I,J,K)
                 az(I,J,K,N) = AZTMP(I,J,K)
              ENDDO
           ENDDO
        ENDDO
       ...
       CALL ADVEC_PRM_TS( UPRM, VPRM, WS, STTMP, N, TF, AXTMP, AYTMP, AZTMP )
...
!
         ...
         END DO
#ifdef SPMD
!      call exchange_3d(dx(1,1,1,n),km,1,1)
!      call exchange_3d(dy(1,1,1,n),km,1,1)
#endif
...
#ifdef SPMD
         call exchange_3d(WKA(1,1,1),km,1,1)
#endif
...
#ifdef SPMD
!      call exchange_3d(dy(1,1,1,n),km,1,1)
!      call exchange_3d(ddy(1,1,1,n),km,1,1)
#endif
...
#endif
#ifdef SPMD
       call exchange_3d(penetrate,km,1,1)
#endif
#endif
...
#endif
#ifdef SPMD
       call exchange_3d(dz(1,1,1,n),km,1,1)
#endif
...
!$OMP PARALLEL DO PRIVATE (J)
             DO J = JSM,JEM
                      NET (1,J,2) =  NET (2,J,2)
                      NET (IMT,J,2) =  NET (IMM,J,2)
             END DO

!$OMP PARALLEL DO PRIVATE (I)
             DO I = 1, IMT
                      NET (I,1,2) =  NET (I,2,2)
                      NET (I,JMT,2) =  NET (I,JEM,2)
             END DO
        end if
#else
        if (nx_proc==1) then
...
#ifdef SPMD
         call exchange_2d(NET(1,1,2),1,1) 
#endif
        ELSE
!$OMP PARALLEL DO PRIVATE (J,I)
            DO J = JSM,JEM
...
!  DFS 2024-02-29
!
!    The original cose as following
!
!!
!!
!        if (nx_proc==1) then
...
!$OMP PARALLEL DO PRIVATE (J)
             DO J = JSM,JEM
                      NET (1,J,1) =  NET (2,J,1)
                      NET (IMT,J,1) =  NET (IMM,J,1)
             END DO

!$OMP PARALLEL DO PRIVATE (I)
             DO I = 1, IMT
                      NET (I,1,1) =  NET (I,2,1)
                      NET (I,JMT,1) =  NET (I,JEM,1)
             END DO
        end if
#else
        if (nx_proc==1) then
...
#ifdef SPMD
         call exchange_2d(NET(1,1,1),1,1) 
#endif
...
#ifdef SPMD
         do j=1,jmt
            if (j_global(j)>=(jst_global+1).and.j_global(j)<=(jst_global+10)) then
#else
            DO J = JSM,JSM +50
#endif
               DO I = 2,IMM
                  TF (I,J,K)= TF (I,J,K) + VIT (I,J,K)* (RESTORE (I,J,K,&
                             N) - ATB (I,J,K,N))*LAMDAN
               END DO
#ifdef SPMD
             endif
#endif
...
!
       ...
       call exchange_3d(vtl(1,1,1),km,1,1)
...
#ifdef SPMD
         call exchange_3d(trend(1,1,1,N),km,1,1) 
#endif
!
!XC
#if (defined TSPAS)
...
! zhangjk 20260613 
! add exchange 
#if (defined OBCDT || defined TS_NUDGE)
#ifdef SPMD
   call exchange_3d_obc(AT(1,1,1,1),km,3,3,imt-2,imt-2,jmt-2,jmt-2,3,3)
   call exchange_3d_obc(AT(1,1,1,2),km,3,3,imt-2,imt-2,jmt-2,jmt-2,3,3)
#endif
#endif


!
! End By Dai in 2017.04
!
!       call energy
!
!
