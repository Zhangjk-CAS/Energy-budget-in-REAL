...
terms = [('edge3adv-mke-era-glorys', 
          'edge3adv-mke-jra-glorys', 
          'edge3adv-mke-jra-licom3', 
          'edgeadv-mke-glorys', 
          'data-licom3high/edgeadv-mke-licom3Global'),
              
         ('edge3adv-era-glorys', 
          'edge3adv-jra-glorys', 
          'edge3adv-jra-licom3', 
          'edgeadv-eke-glorys', 
          'data-licom3high/edgeadv-licom3Global'),
              
         ('edge2ape-era-glorys', 
          'edge2ape-jra-glorys', 
          'edge2ape-jra-licom3', 
          'edgeape-mke-glorys', 
          'data-licom3high/edgeape-licom3Global'),
              
         ('edge3eape-era-glorys', 
          'edge3eape-jra-glorys', 
          'edge3eape-jra-licom3', 
          'edgeeape-glorys', 
          'data-licom3high/edgeeape-licom3Global'),] # for mke 
...
for nnterm in range(4):
    ...
    for nndata in range(5): 
        ...
        if nnterm in [0,2]:
            vare_pu = (varpu_e[0, :, lat_loc[0]:lat_loc[1]]) 
            varw_pu = (varpu_w[0, :, lat_loc[0]:lat_loc[1]]) 
            vars_pu = (varpu_s[0, :, lon_loc[0]:lon_loc[1]]) 
            varn_pu = (varpu_n[0, :, lon_loc[0]:lon_loc[1]]) 
        elif nnterm in [1,3]:
            vare_pu = np.nanmean(varpu_e[0:ntime, :, lat_loc[0]:lat_loc[1]], axis=0) 
            varw_pu = np.nanmean(varpu_w[0:ntime, :, lat_loc[0]:lat_loc[1]], axis=0) 
            vars_pu = np.nanmean(varpu_s[0:ntime, :, lon_loc[0]:lon_loc[1]], axis=0) 
            varn_pu = np.nanmean(varpu_n[0:ntime, :, lon_loc[0]:lon_loc[1]], axis=0) 
        ...
        axs[nnterm1,nnterm2].plot(lat, vare_itg, color=colors[nndata], alpha=0.2, 
                                  linewidth=1.5, linestyle=linesty[nndata])

        kernel_y = np.ones(30) / 30
        vare_itg = convolve1d(vare_itg, weights=kernel_y, axis=0, mode='nearest')
        if nndata < 3:
            axs[nnterm1,nnterm2].plot(lat, vare_itg, color=colors[nndata], linewidth=0.9, linestyle='-')
        else:
            axs[nnterm1,nnterm2].plot(lat, vare_itg, color=colors[nndata], linewidth=0.9, linestyle='--')
        ...
        # if nndata == 0:
        #     axs[nnterm1,nnterm2].set_title(tn[nnterm], loc='left', pad=0.1)

axs[0,0].plot(lat, vare_itg+999, color=colors[0], linewidth=0.9, linestyle='-', label=ln[0])
axs[0,0].plot(lat, vare_itg+999, color=colors[1], linewidth=0.9, linestyle='-', label=ln[1])
axs[0,0].plot(lat, vare_itg+999, color=colors[2], linewidth=0.9, linestyle='-', label=ln[2])
axs[0,0].plot(lat, vare_itg+999, color=colors[3], linewidth=0.9, linestyle='--', label=ln[3])
axs[0,0].plot(lat, vare_itg+999, color=colors[4], linewidth=0.9, linestyle='--', label=ln[4])
axs[0,0].legend(loc='lower left', ncol=2, frameon=False, fontsize=10)

axs[0,0].set_ylim(-0.9,0.3)
axs[0,0].set_xticks([])
axs[0,1].set_xticks([])
axs[1,0].set_xticks(range(-10,45,10))
axs[1,1].set_xticks(range(-10,45,10))
axs[0,0].set_xlim(lat_range)
axs[1,0].set_xlim(lat_range)
axs[0,1].set_xlim(lat_range)
axs[1,1].set_xlim(lat_range)

latticklabel = ['10°S',  '0°N', '10°N', '20°N', '30°N', '40°N']
axs[1,0].set_xticklabels(latticklabel)
axs[1,1].set_xticklabels(latticklabel)
axs[0,0].set_ylabel('×10⁵W/m')
axs[1,0].set_ylabel('×10⁵W/m')

pnstr2 = f'edgework-itg-adv'
