...
# pnstrs = [f'edgepwork-mke-eg-dailyBC-all']
# paths = ["../output-era2/fort.22/ave/"]
pnstrs = [f'TH/edgeup-glorys-all']
paths = ["../exe/boundary-data-glorys-daily/ave/"]
for nforce in range(1):
    ...
    for item in sorted(os.listdir(nowdir)):
        ...
        if any(key in item for key in key_strs):
            ...
            var0 = ds[vn].values[0,1:,:,:] - um # mean u was set to zero 
            lon0 = ds['lon'].values
            lat0 = ds['lat'].values
            ...
            var313 = varrho[-1,~np.isnan(varrho[-1,:,:])]
            areas313 = areas[-1,~np.isnan(varrho[-1,:,:])]
            # x = 0;xx = 0
            # print(np.nansum(areas * varrho, axis=(1,2))/np.nansum(areas, axis=(1,2)))
            # for ii in range(len(var313)):
            #     x += var313[ii]*areas313[ii]
            #     xx += areas313[ii]
            # print([x, xx, x/xx])
            
            print(np.nanmean(varrho,axis=(1,2)))
