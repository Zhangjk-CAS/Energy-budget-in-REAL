...
# lon_range=[100, 148];lat_range=[-15,45] # the reference rho range 
lon_range=[100, 150];lat_range=[-15,45]
...
for nforce in range(3):
    ...
    for item in sorted(os.listdir(nowdir)):
        ...
        if any(key in item for key in key_strs):
            ...
            # fn3= fn.replace('ave/'+vn,'TH/ave/rho')
            ...
            ape = g**2 * rhoa**2 / (2 * rho0 * N2[:, np.newaxis, np.newaxis]) 
            # print(rhoref)
            # print(N2)
            ...
            deudx_e[number_day, :, :] = (varpu[:, :,-21]) 
            deudx_w[number_day, :, :] = (varpu[:, :, 0]) 
            devdy_s[number_day, :, :] = (varpv[:, -1,:]) 
            devdy_n[number_day, :, :] = (varpv[:,  0,:]) 
            print(varpu.shape)
            print(np.nansum(areas))

            print([np.nansum(deudx_e[number_day, :]),np.nansum(deudx_w[number_day, 0, :]),
